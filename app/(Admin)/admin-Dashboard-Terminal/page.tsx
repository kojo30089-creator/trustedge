    ...
    const loadPrices = async () => {
      ...
      try {
        ...
        if (snap.exists()) {
          const d = snap.data();
          setStockForm({
            spacex: d.spacex?.toString() ?? "",
            neuralink: d.neuralink?.toString() ?? "",
            boring: d.boring?.toString() ?? "",
          });
        }
      } catch (e) {
        console.error(e);
    ...
    >
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div>
              <Link
                href="/dashboard"
                className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-blue-600 dark:text-blue-400 mb-6 hover:opacity-70 transition-opacity"
              >
                <FaArrowLeftLong /> Preview Terminal
              </Link>
              <p className="text-[10px] font-mono text-slate-400 uppercase tracking-[0.4em] mb-2">
                System_Control_Node
              </p>
            ...
            <div className="flex gap-4">
              <Button
                onClick={openModal}
                className="h-14 px-8 rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold shadow-xl shadow-slate-900/10 hover:scale-105 transition-transform active:scale-95 gap-2"
              >
          ...
          >
            <div className="bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 p-6 rounded-[24px]">
              ...
              <p className="text-4xl font-bold text-slate-900 dark:text-white font-mono tracking-tighter">
                {users.length}
              </p>
      ...
      <section className="px-4 md:px-10 mt-12">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <motion.div
            variants={itemVariants}
            className="bg-white dark:bg-[#121214] border border-slate-100 dark:border-white/5 rounded-[32px] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none"
          >
            <div className="overflow-x-auto">
              <Table>
                <TableHeader className="bg-slate-50/50 dark:bg-white/[0.02] border-b border-slate-100 dark:border-white/5">
                  <TableRow className="hover:bg-transparent">
                    <TableCell className="text-[10px] font-bold uppercase tracking-widest text-slate-400 py-4 px-6">
                      Identity
                    </TableCell>
                    <TableCell className="text-[10px] font-bold uppercase tracking-widest text-slate-400 py-4">
                      Status
                    </TableCell>
                    <TableCell className="text-[10px] font-bold uppercase tracking-widest text-slate-400 py-4 text-center">
                      Liquidity
                    </TableCell>
                    <TableCell className="text-[10px] font-bold uppercase tracking-widest text-slate-400 py-4">
                      Location
                    </TableCell>
                    <TableCell className="text-[10px] font-bold uppercase tracking-widest text-slate-400 py-4 text-right px-6">
                      Actions
                    </TableCell>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  <AnimatePresence mode="popLayout">
                    {filteredUsers.map((user) => (
                      <TableRow
                        key={user.id}
                        className="group hover:bg-slate-50 dark:hover:bg-white/[0.01] border-b border-slate-50 dark:border-white/[0.02] transition-colors cursor-default"
                      >
                        <TableCell className="py-5 px-6">
                          <Link
                            href={`/admin-Dashboard-Terminal/user-profiles/${user.id}`}
                          ...
                          >
                            <Avatar className="h-10 w-10 rounded-xl border border-slate-100 dark:border-white/10">
                              ...
                              />
                              <AvatarFallback className="bg-slate-100 rounded-xl dark:bg-slate-800 text-xs font-bold text-slate-400">
                                {user.name.substring(0, 2).toUpperCase()}
                              </AvatarFallback>
                            </Avatar>
                            <div className="flex flex-col">
                              <span className="text-sm font-bold text-slate-900 dark:text-white leading-tight group-hover:text-blue-500 transition-colors">
                                {user.name}
                              </span>
                        ...
                        <TableCell className="text-right px-6">
                          <Link
                            href={`/admin-Dashboard-Terminal/user-profiles/${user.id}`}
                            className="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 dark:bg-white/5 text-slate-400 hover:text-blue-500 dark:hover:text-white transition-colors"
                          >
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-md">
        <div className="p-8 bg-white dark:bg-[#09090b] rounded-[32px]">
          ...
          <p className="text-sm text-slate-500 mb-8 font-medium">
            Update internal asset valuations for share calculation.
          </p>
          ...
          <form onSubmit={handleSaveStockPrices} className="space-y-6">
            {[
              ...
              {
                id: "neuralink",
                label: "Neuralink (NRLK)",
                val: stockForm.neuralink,
              },
              {
                id: "boring",
                label: "The Boring Co. (TBC)",
                val: stockForm.boring,
              },
            ].map((stock) => (
              <div key={stock.id} className="space-y-1.5">
                <Label
                  htmlFor={stock.id}
                  className="text-[10px] uppercase font-bold tracking-widest text-slate-400 ml-1"
                >
                  {stock.label}
                </Label>
                <div className="relative">
                  ...
                  <input
                    ...
                    value={stock.val}
                    onChange={(e) =>
                      setStockForm({ ...stockForm, [stock.id]: e.target.value })
                    }
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button
                ...
                className="flex-[2] h-12 rounded-xl bg-blue-600 text-white font-bold shadow-lg shadow-blue-500/20"
              >
